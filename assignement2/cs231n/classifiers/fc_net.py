...
class TwoLayerNet(object):
	...
	** The architecure should be affine - relu - affine - softmax.
	...
	def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
		...
		self.params['b1'] = np.zeros([hidden_dim]) # initialize to zero
		self.params['W2'] = np.random.normal(0, weight_scale, [hidden_dim, num_classes])
		self.params['b2'] = np.zeros([num_classes]) # initialize to zero
	...
	def loss(self, X, y=None):
		...
		bs = X.shape[0]
		# reshape input
		flat_X = np.reshape(X, [bs, -1])
		# affine 1
		aff1_out, aff1_cache = affine_forward(flat_X, self.params['W1'], self.params['b1'])
		# ReLu
		relu1_out, relu1_cache = relu_forward(aff1_out)
		aff2_out, aff2_cache = affine_forward(relu1_out, self.params['W2'], self.params['b2'])
		# scores to be forwarded to Softmax
		scores = np.copy(aff2_out)
		...
		dx2, dw2, db2 = affine_backward(dsoft, aff2_cache)
...
class FullyConnectedNet(object):
	...
	def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
		...
		# Initialise the weights and biases by looping through all of them and given parameters
	...
	def loss(self, X, y=None):
		...
		X = np.reshape(X, [batch_size, -1]) # flatten input
		
		# affine ReLU forward passes
		for i in range(self.num_layers-1):
