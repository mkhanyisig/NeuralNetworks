...
def affine_backward(dout, cache):
	...
	# Reshape each input in batch to a column vector.
...
def relu_backward(dout, cache):
	...
	# only backprop to non-negative elements of x
	mask = (x >= 0)
	dx = dout * mask
...
def batchnorm_forward(x, gamma, beta, bn_param):
	...
	if mode == 'train':
		...
		# minibatch (x) statistics to compute the mean and variance for each x[i] into column vector
		sample_mean = np.mean(x, axis = 0)	# shape (N,)
		sample_var = np.var(x, axis = 0) # shape (N,)
...
def batchnorm_backward(dout, cache):
	...
	# retrive variables from cache
	x, gamma, beta, x_hat, sample_mean, sample_var, eps = cache
	N, D = x.shape
	
	# this took a while understanding, but the backprop graph equations given in the Loffe and Szegedy paper
	# computational graph
	dx_1 = gamma * dout
...
def layernorm_forward(x, gamma, beta, ln_param):
	...
	# Normalize the incoming data by dimension
	dim_mean = np.mean(x, axis=1, keepdims=True) # shape (D,1)
	dim_var = np.var(x, axis=1, keepdims=True)	# shape (D,1)
	
	# corrected mean
	x_zeroed = x - dim_mean			 # shape (N, D)
	
	varx_sqrt= np.sqrt(dim_var + eps)
	x_hat = x_zeroed / np.sqrt(dim_var + eps) # (N, D)
...
def layernorm_backward(dout, cache):
	...
	dx1_1 = dx0 / varx_sqrt
	dx1_2 = np.sum(dx0 * x_zeroed, axis=1, keepdims=True)  
	dx2 = dx1_2 / (-varx_sqrt**2)
	dx3 = dx2 / (2 * varx_sqrt)
	dx4 = dx3 * np.ones([N, D]) / D
	dx5 = dx4 * 2 * x_zeroed
	dx6_1 = (dx5 + dx1_1)
	dx6_2 = -1 * np.sum(dx5 + dx1_1, axis=1, keepdims=True)	 
...
def conv_forward_naive(x, w, b, conv_param):
	...
	for hn, hi in enumerate(range(0, H, stride)):
		for wn, wi in enumerate(range(0, W, stride)):
			...
			dummy0, dummy1, check_hi, check_wi = field.shape
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
	...
	(N, C, H, W) = x.shape
	# conv feature map normalization
